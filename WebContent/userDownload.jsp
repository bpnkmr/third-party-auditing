    ...
    <tbody>
      ...
      <%
		 ...
		 UserData ud=new UserData();
				ResultSet rs1= ud.DownloadList(id);
			if(rs1!=null)
			{int i=1;
				while(rs1.next())
				{
       ...
       <%--  <td><a name="file" class="btn btn-launch" data-toggle="modal" href="#" onClick="goTranDeleteBill('<%=rs1.getString(1)%>','<%=rs1.getString(2)%>')" > Download</a></td>
        --%> 
         <td>
          <a href="DownloadFileServlet?name=<%=rs1.getString(2)%>&id=<%=rs1.getString(1) %>"> <span class="label label-success">Download</span> </a>
           </td>
          <td>
               <a href="UserDeleteServlet?name=<%=rs1.getString(2) %>&id=<%=rs1.getString(1) %>"><span class="label label-primary">Delete</span> </a>
  		</td>
      </tr>
     <%i++;
			}
